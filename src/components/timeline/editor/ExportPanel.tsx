...
export function ExportPanel({ isOpen, onClose }: Props) {
  ...
  const getProjectDuration = () => {
    let maxEnd = 0
    ;(tracks || []).forEach((t) => { if (!t?.clips) return; t.clips.forEach((c) => { if (!c) return; const end = c.startTime + c.duration; if (end > maxEnd) maxEnd = end }) })
    return maxEnd || 5
  }

  const getTotalClips = () => (tracks || []).reduce((sum, t) => sum + (t?.clips?.length || 0), 0)
  ...
  const startExport = async () => {
    ...
    const videoTrack = safeTracks.find((t) => t.type === 'video')
    const imageTrack = safeTracks.find((t) => t.type === 'image')
    const textTrack = safeTracks.find((t) => t.type === 'text')
    const audioTrack = safeTracks.find((t) => t.type === 'audio')
